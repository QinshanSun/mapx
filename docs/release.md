...
MapX uses GitHub Actions to build stable GitHub Releases from Git tags. The release track now has two parts:
...
MapX checks `https://github.com/QinshanSun/mapx/releases/latest/download/latest.json` for signed updates. For China-based users, GitHub connectivity may be unreliable. Startup check failures are logged silently and do not block the main workspace. Manual check, download, install, or signature failures are shown in the app with an option to open the GitHub Releases download page.
