...
Use this checklist before closing the `V1 Auto Update` milestone or issue `QA-009`. Dev-server checks and mock updater tests are not enough; the updater must be verified from packaged Tauri apps.

## Preconditions

- GitHub Actions can start runners for this repository.
...
- Draft and prerelease releases, if present, must not be the latest stable release consumed by the app.
...
5. Confirm `启动时自动检查更新` is enabled by default unless the test case intentionally disables it.
6. Click `检查更新`.
...
4. Confirm `启动时自动检查更新` is enabled by default unless the test case intentionally disables it.
5. Click `检查更新`.
...
- Manual check failure must show a user-friendly error and an `打开下载页面` fallback.
